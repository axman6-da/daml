...
class EqualitySpec extends WordSpec with Matchers with TableDrivenPropertyChecks {
  ...
  private def left(x: SValue) = SVariant(VariantTypeCon, VariantCon1, 0, x)
  private def right(x: SValue) = SVariant(VariantTypeCon, VariantCon2, 1, x)
  ...
  "Equality.areEqual" should {
    ...
    val x1 = right(SInt64(1))
    val x2 = right(SInt64(2))
    val u = left(fun)
    ...
    "shortcut failure in gen map" in {
      ...
      val k1 = x1
      val k2 = x2
      assert(Ordering.compare(x1, x2) < 0)
